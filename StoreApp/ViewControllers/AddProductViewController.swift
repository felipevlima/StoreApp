...
class AddProductViewController: UIViewController {
    ...
    @objc func saveButtonPressed(_ sender: UIBarButtonItem) {
        guard let title = titleTextField.text, let price = Double(priceTextField.text ?? "0.00"), let description = descriptionTextView.text,
              let imageUrl = imageURLTextField.text, let productImageUrl = URL(string: imageUrl), let category = selectedCategory else {
            return
        }
