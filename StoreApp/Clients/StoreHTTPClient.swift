...
class StoreHTTPClient {
    ...
    func getProductsByCategory(cateogryId: Int) async throws -> [Product] {
        let (data, response) = try await URLSession.shared.data(from: URL.productsByCategory(cateogryId))
    ...
    // MARK: - POST Request
    /// É assim que se faz uma requisição POST no swift
